...
5. **✅ 완료 — Kết bạn gần bạn** (`/ket-ban`, 2026-07-04 추가)
   외로움 해소용 "주변 사람과 친구 되기" 기능. opt-in 방식(기본 비공개, 사용자가 직접 위치 공유 켜야 함), 서로 "인사"를 보내야 매칭("Đã kết nối")됨. 타겟 유저 중 미성년 자녀·임산부 등 취약 계층이 포함되어 있어 **데이팅 앱이 아닌 "친구 만들기" 톤을 유지**하고, 안전 수칙(공개 장소에서 만나기, 지인에게 알리기, 송금 금지)을 화면에 항상 노출한다.
