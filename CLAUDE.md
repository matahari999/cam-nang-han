...
5. **✅ 완료 — Kết bạn gần bạn** (`/ket-ban`, 2026-07-04 추가, 2026-07-04 저녁 레이더 지도 추가)
   외로움 해소용 "주변 사람과 친구 되기" 기능. opt-in 방식(기본 비공개, 사용자가 직접 위치 공유 켜야 함), 서로 "인사"를 보내야 매칭("Đã kết nối")됨. 타겟 유저 중 미성년 자녀·임산부 등 취약 계층이 포함되어 있어 **데이팅 앱이 아닌 "친구 만들기" 톤을 유지**하고, 안전 수칙(공개 장소에서 만나기, 지인에게 알리기, 송금 금지)을 화면에 항상 노출한다. `components/ketban/NearbyRadarMap.tsx`가 카카오맵 위에 자신 위치(레이더 펄스 애니메이션)+주변 사람 핀을 표시 — **다른 사람의 좌표는 서버에서 100~300m 무작위로 흐려서(`nearby_friends` RPC의 `fuzzy_lat/fuzzy_lng`) 반환**, 실제 집/직장 위치 노출 방지. 카카오맵 비즈 앱 미승인 상태라 현재는 "지도 준비 중" 안내만 뜨고 리스트만 동작 — 승인되면 자동으로 지도 활성화(코드 수정 불필요).
6. **✅ 완료 — Phiên dịch Việt–Hàn** (`/phien-dich`, 2026-07-04 추가)
   독립 통번역기 툴(베트남어↔한국어). Gemini로 자연스러운 번역, 결과를 큰 글씨로 표시 + 브라우저 내장 TTS(`speechSynthesis`)로 읽어주기. 병원/주민센터/시댁 등 대면 상황에서 화면을 보여주는 용도 — 상대방이 앱을 깔 필요 없음. 여가부 2024 다문화가족 실태조사에서 결혼이민자가 가장 필요로 한 서비스가 "통번역"이라는 근거로 추가.
